...
_apply_theme() {
  ...
  # -- status line -------------------------------------------------------
    ...
    # set-titles-string (message in terminal tab)
    ...
    # seperator
      ...
      tmux_conf_theme_left_window_separator_main=$(_decode_unicode_escapes "${tmux_conf_theme_left_window_separator_main-}")
      tmux_conf_theme_left_window_separator_sub=$(_decode_unicode_escapes "${tmux_conf_theme_left_window_separator_sub-|}")
      tmux_conf_theme_right_window_separator_main=$(_decode_unicode_escapes "${tmux_conf_theme_right_window_separator_main-}")
      tmux_conf_theme_right_window_separator_sub=$(_decode_unicode_escapes "${tmux_conf_theme_right_window_separator_sub-|}")
...
EOF
  ...
  case "$status_left $status_right" in
    *'#{battery_status}'*|*'#{battery_bar}'*|*'#{battery_hbar}'*|*'#{battery_vbar}'*|*'#{battery_percentage}'*)
      ...
      status_right="#(cut -c1- $BLADE_TMUX_CONFIG_FILE_DIR/gpakosz-tmux.sh | sh -s _battery)$status_right"
      ;;
  esac
  ...
  case "$status_left $status_right" in
    *'#{uptime_d}'*|*'#{uptime_h}'*|*'#{uptime_m}'*|*'#{uptime_s}'*)
      ...
      status_right=$(echo "$status_right" | sed -E \
        ...
        -e 's/#\{(\?)?uptime_m/#\{\1@uptime_m/g' \
        -e 's/#\{(\?)?uptime_s/#\{\1@uptime_s/g')
      status_right="#(cut -c1- $BLADE_TMUX_CONFIG_FILE_DIR/gpakosz-tmux.sh | sh -s _uptime)$status_right"
      ;;
  esac
  ...
  case "$status_left $status_right" in
    *'#{loadavg}'*)
      ...
      status_right=$(echo "$status_right" | sed -E \
        -e 's/#\{(\?)?loadavg/#\{\1@loadavg/g')
      status_right="#(cut -c1- $BLADE_TMUX_CONFIG_FILE_DIR/gpakosz-tmux.sh | sh -s _loadavg)$status_right"
      ;;
  esac

  status_left=$(echo "$status_left" | sed 's%#{circled_session_name}%#(cut -c1- $BLADE_TMUX_CONFIG_FILE_DIR/gpakosz-tmux.sh | sh -s _circled #S)%g')
  status_right=$(echo "$status_right" | sed 's%#{circled_session_name}%#(cut -c1- $BLADE_TMUX_CONFIG_FILE_DIR/gpakosz-tmux.sh | sh -s _circled #S)%g')
  ...
  tmux  set -g status-left-length 1000 \; set -g status-left "$(_decode_unicode_escapes "$status_left")" \;\
    ...
    # window-status
      ...
      tmux setw -g window-status-separator "$tmux_conf_theme_window_status_separator"
